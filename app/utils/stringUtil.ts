...
/* 隐藏手机号中间部分 */
export const hidePhoneNum = (phone: string) => {
 ...
 * @returns {string}
 */
export const handleStr = (str: string) => {
  return !!str ? str : '';
};
 ...
 * @param str
 */
export const handleBankCardMask = (str: string) => {
  let cardNum = str;
  if (str && str.length >= 5) {
...
export const isImage = (filename: string) => /.(gif|jpg|jpeg|png)$/.test(filename ? filename.toLowerCase() : filename);
