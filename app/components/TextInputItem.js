...
export default class TextInputItem extends Component {
  static propTypes = {
    ...
    picVerifyCodeStyle: PropTypes.oneOfType([
      PropTypes.object,
      PropTypes.number,
    ]) /* 图形验证码样式(可选) */,
    picVerifyCodeClick: PropTypes.func /* 图形验证码点击事件 */,
    verifyCodeUrl:
      PropTypes.string /* 获取验证码的接口地址(后续传值使用,如果isShowVerifiyCode==true, 此项需填写) */,
    smsType:
      PropTypes.string /* 验证码类型(后续传值使用,如果isShowVerifiyCode==true, 此项需填写) */,
  ...
  renderExpress = () => {
    ...
    return (
      <TouchableOpacity
        style={{padding: px(20)}}
        onPress={this.setInputVisible}>
        <Image
          style={styles.expressStyle}
          resizeMode="contain"
          source={secureIcon}
        />
      </TouchableOpacity>
  ...
  render() {
    ...
    let VerifiyCodeComponent = isShowVerifiyCode
      ? this.renderVerifiyCode()
      : null;
    let picVerifiyCodeComponent = isShowPicVerifiyCode
      ? this.renderPicVerifiyCode()
      : null;
