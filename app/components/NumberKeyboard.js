...
export default class NumberKeyboard extends Component {
  ...
  state = {
    inputResult: '',
    grassTransY: new Animated.Value(
      isIphoneX() ? px(434 + iphoneXSafeAreaBottom) : px(434),
    ),
    isHideKeyboard: true,
    currentAnimatedProgress: isIphoneX()
      ? px(434 + iphoneXSafeAreaBottom)
      : px(434),
  };
   ...
   * @param inputItem
   */
  setInputResult = (inputItem, onInputChange, onConfirmClick) => {
    let inputTemp = this.state.inputResult;
    if (inputItem === '.') {
      if (inputTemp.indexOf('.') > -1) return;
      ...
      if (inputTemp.length === 1) {
        ...
        this.setState(
          {inputResult: inputTemp.substring(0, inputTemp.length - 1)},
          () => {
            onInputChange(this.state.inputResult, inputItem);
          },
        );
      ...
      if (pointIndex > -1) {
        let tempLength = inputTemp.substring(pointIndex + 1, inputTemp.length)
          .length;
        if (tempLength === 2) return;
        if (tempLength === 1) {
        ...
        if (inputTemp.replace('0', '') === '') {
          ...
          if (!this.checkMaxLength(inputTemp)) return;
      ...
      if (inputTemp.replace('0', '') === '') {
        ...
        if (pointIndex > -1) {
          if (
            inputTemp.substring(pointIndex + 1, inputTemp.length).length === 2
          )
            return;
        }
        if (!this.checkMaxLength(inputTemp)) return;
    ...
    return (
      <Animated.View
        style={{
          height:
            (isIphoneX() ? px(434 + iphoneXSafeAreaBottom) : px(434)) -
            this.state.currentAnimatedProgress,
        ...
        <View style={styles.containerStyle}>
          ...
          <View style={{flex: 1}}>
            <TouchableOpacity
              ...
              <Image
                style={styles.deleteKeyStyle}
                source={require('../assets/images/common/keyboard_clear.png')}
              />
            </TouchableOpacity>
            ...
            <TouchableOpacity
              ...
              style={[
                styles.singleKeyStyle,
                {marginRight: 0, marginTop: 1, flex: 0.98},
              ]}>
