...
const TabBar = props => {
  ...
  return (
    <View style={[styles.tab, {backgroundColor}]}>
      {left ? (
        <View style={[styles.tabLeft, {width: leftWidth}]}>{left}</View>
      ) : (
        undefined
      )}
      <TabItem {...props} />
      {right ? (
        <View style={[styles.tabRight, {width: rightWidth}]}>{right}</View>
      ) : (
        undefined
      )}
    </View>
      ...
      transform: [
        {
          translateX: this.props.scrollValue.interpolate({
            ...
            outputRange: [
              0,
              (containerWidth - leftWidth - rightWidth) / numberOfTabs,
            ],
          }),
        },
      ],
...
export default props => {
  ...
  return (
    <ScrollableTabView
      {...tabProps}
      renderTabBar={() => <TabBar {...otherProps} />}>
      {children}
    </ScrollableTabView>
  );
};
